...
const ApplicantsTable = ({ record, loading }) => {
    ...
    return (
      <NameBodyStyles>
        <div className="avatar">
          {record?.studentname[0]?.avatar && (
            <img src={record?.studentname?.avatar} alt="" />
          )}
        </div>
        <div className="name-info">
          <h5>{record?.studentname[0]?.name}</h5>
  ...
  const mandalBodyTemplate = (record) => {
    return (
      <p>
        {record?.studentname[0]?.mandal ? record?.studentname[0]?.mandal : "-"}
      </p>
    );
  };
