...
const CollegeTable = () => {
   ...
   const paymentStatus = (record) => {
      return <p className={`payment-status ${getPStatus(record)}`}>{record.pStatus}</p>;
   };
   ...
   const onboardStatus = (record) => {
      return <p className={`onboard-status ${getOStatus(record)}`}>{record.oStatus}</p>;
   };
   ...
   return (
      <DatatableStyles>
         <DataTable value={record} size="large" paginator rows={5} rowsPerPageOptions={[5, 8, 10, 20, 50]} selectionMode="checkbox" selection={selectedProducts} onSelectionChange={(e) => setSelectedProducts(e.value)} rowHover dataKey="collegeid" onRowClick={handleRowClick} header={header} filters={filters} onFilter={(e) => setFilters(e.filters)} tableStyle={{ minWidth: "60rem" }}>
            <Column selectionMode="multiple" exportable={false} className="tableText" headerStyle={{ width: "3rem" }}></Column>
            <Column className="tableText" field="collegename" header="College's"></Column>
            <Column className="tableText text-center" field="address" header="City" body={cityBodyTemplate}></Column>
            <Column className="tableText text-center" field="total_admission" header="Total Admission"></Column>
            <Column className="tableText text-center" field="applied_admission" header="Applied Admission"></Column>
            <Column className="tableText text-center" field="oStatus" header="Onboard Status" body={onboardStatus}></Column>
            <Column className="tableText text-center" style={{ minWidth: "150px" }} field="cType" header="College Type 10+2 or 12+3"></Column>
            <Column className="tableText text-center" field="rStatus" header="Review Status" body={ratingBodyTemplate}></Column>
            <Column className="tableText text-center" field="pStatus" header="Payment Status" body={paymentStatus}></Column>
         </DataTable>
      </DatatableStyles>
   );
};
