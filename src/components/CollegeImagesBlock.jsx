...
const CollegeImagesBlock = () => {
   ...
   const [collegeImage, setCollegeImage] = useState([{ image: null, imagePreview: null, title: "" }]);
   ...
   const handleChange = (e, i) => {
      ...
      if (name == "image") {
         ...
         const fileName = `college\$${uuidv4()}\$${localStorage.getItem("collegeId")}.${fileExt}`;
   ...
   const handleAddField = (e) => {
      e.preventDefault();
      setCollegeImage([...collegeImage, { image: null, imagePreview: null, title: "" }]);
   };
               ...
               {collegeImage.map((item, i) => (
                  <ImageItemStyle key={i}>
                     <div className="clg-image-block">
                        <div className="clg-img">{item.imagePreview ? <img src={item.imagePreview} alt="" /> : <p>Add Image*</p>}</div>
                        <div className="img-ctas">
                           <label className="edit-cta" htmlFor={`college-image-${i}`}>
                              <AddIconBig />
                              <p>{item.image ? "Edit" : "Add"}</p>
                           </label>
                           <input type="file" accept="image/*" required name="image" id={`college-image-${i}`} onChange={(e) => handleChange(e, i)} style={{ display: "none" }} />
                           {item.image && (
                     ...
                     <div className="clg-img-title">
                        <input type="text" required name="title" value={item.title} onChange={(e) => handleChange(e, i)} placeholder="Name" />
                     </div>
                  </ImageItemStyle>
               ))}
